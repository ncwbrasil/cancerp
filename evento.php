...
<div class="wrapper" id="eventos">
    ...
    if($rows > 0 )
    {
        while($row = mysql_fetch_array($query))
        {
            ...
            <div class='titulo_evento'>".$row['eve_titulo']."</div>
            <div class='data_evento'>publicado em: ".implode("/",array_reverse(explode("-",$row['eve_data'])))."</div><br>
            <div class='anexo'>";
			if(strpos($final,'pdf') !== false)
			{ 
				echo "Anexo: <a target='_blank' href='".$row['eve_anexo']."'><img src='imagens/icon-pdf.png' border='0' valign='bottom'></a>";
			}
			elseif(strpos($final,'jpg') !== false || strpos($final,'png') !== false)
			{ 
				echo "Anexo: <a target='_blank' href='".$row['eve_anexo']."'><img src='imagens/icon-img.png' border='0' valign='bottom'></a>";
			}
			echo "</div>
            <div class='descricao_evento'>".$row['eve_descricao']."</div>
			<div class='linha'>&nbsp;</div>					
            ";
        }
    }
