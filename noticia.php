...
<div class="wrapper" id="noticias">
	<?php 
    $nt_id = $_GET['nt_id'];
    $sql = "SELECT * FROM noticias WHERE nt_id = $nt_id ";
    ...
    if($rows > 0 )
    {
        while($row = mysql_fetch_array($query))
        {
            ...
            <div class='titulo_noticia'>".$row['nt_titulo']."</div>
            <div class='data_noticia'>publicado em: ".implode("/",array_reverse(explode("-",$row['nt_data'])))."</div><br>
            <div class='anexo'>";
			if(strpos($final,'pdf') !== false)
			{ 
				echo "Anexo: <a target='_blank' href='".$row['nt_anexo']."'><img src='imagens/icon-pdf.png' border='0' valign='bottom'></a>";
			}
			elseif(strpos($final,'jpg') !== false || strpos($final,'png') !== false)
			{ 
				echo "Anexo: <a target='_blank' href='".$row['nt_anexo']."'><img src='imagens/icon-img.png' border='0' valign='bottom'></a>";
			}
			echo "</div>
            <div class='descricao_noticia'>".$row['nt_descricao']."</div>
			";
			if($row['nt_video'] == '')
			{ }
			else
			{
				...
				echo "
				<div id='video'>
				<iframe width='600' height='380' src='//youtube.com/embed/$matches[1]' frameborder='0' allowfullscreen></iframe>
				</div>
			";
			}
			echo "
