...
<div class="wrapper" id="noticias">
	<div class='noticia'>Notícias</div>
	<?php 
    $sql = "SELECT * FROM noticias WHERE nt_status = 1 AND nt_data <= '".date('Y-m-d')."' ORDER BY nt_data DESC";
   ...
   if($rows > 0 )
    {
        ...
        {
            echo "
            <div class='titulo_noticia'><a href='noticia.php?nt_id=".$row['nt_id']."'>".$row['nt_titulo']."</a></div>
            <div class='data_noticia'>publicado em: ".implode("/",array_reverse(explode("-",$row['nt_data'])))."</div>
            <div class='descricao_noticia'>".truncate(strip_tags(preg_replace("/<img[^>]+\>/i", " ",str_replace("<br />"," ",str_replace("</p>"," ",str_replace("<p>"," ",$row['nt_descricao']))))),280)."</div>
            <div class='continuar'><a href='noticia.php?nt_id=".$row['nt_id']."'>>> continuar lendo</a></div>
