...
<div class="wrapper" id="publicacoes">
	<div class='publicacao'>Publicações</div>
	<?php 
    $sql = "SELECT * FROM publicacoes WHERE pub_status = 1 AND pub_data <= '".date('Y-m-d')."' ORDER BY pub_data DESC";
   ...
   if($rows > 0 )
    {
        ...
        {
            echo "
            <div class='titulo_publicacao'><a href='publicacao.php?pub_id=".$row['pub_id']."'>".$row['pub_titulo']."</a></div>
            <div class='data_publicacao'>publicado em: ".implode("/",array_reverse(explode("-",$row['pub_data'])))."</div>
            <div class='descricao_publicacao'>".truncate(strip_tags(preg_replace("/<img[^>]+\>/i", " ",str_replace("<br />"," ",str_replace("</p>"," ",str_replace("<p>"," ",$row['pub_descricao']))))),280)."</div>
            <div class='continuar'><a href='publicacao.php?pub_id=".$row['pub_id']."'>>> continuar lendo</a></div>
